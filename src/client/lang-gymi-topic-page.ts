...
const copy: Record<Language, LangCopy> = {
  ...
  de: {
    ...
    title: "Vereinfache die Terme so weit wie möglich",
    description:
      "Übe Terme mit Klammern, Produkten, Brüchen und Wurzeln. Bei einer falschen Antwort zeigt das System die Lösung Schritt für Schritt und schlägt ein ähnliches Beispiel vor.",
...
function isAnswerCorrect(input: string, task: LangTask): boolean {
...
export class LangGymiTopicPage {
  ...
  private bindEvents(): void {
    this.root.querySelector<HTMLInputElement>("#lang-topic-answer")?.addEventListener("input", (event) => {
    ...
    this.root.querySelector<HTMLButtonElement>("#lang-topic-back-button")?.addEventListener("click", () => {
      this.onBack();
    });

    this.root.querySelector<HTMLButtonElement>("#lang-topic-check-button")?.addEventListener("click", () => {
      this.handleCheck();
    });

    this.root.querySelector<HTMLButtonElement>("#lang-topic-solution-button")?.addEventListener("click", () => {
      this.showSolution = !this.showSolution;
      this.render();
    });

    this.root.querySelector<HTMLButtonElement>("#lang-topic-prev-button")?.addEventListener("click", () => {
      if (this.currentIndex > 0) {
        this.resetTaskView(this.currentIndex - 1);
      }
    });

    this.root.querySelector<HTMLButtonElement>("#lang-topic-next-button")?.addEventListener("click", () => {
      if (this.currentIndex < TASKS.length - 1) {
        this.resetTaskView(this.currentIndex + 1);
      }
    });

    this.root.querySelector<HTMLButtonElement>("#lang-topic-reset-button")?.addEventListener("click", () => {
      this.resetTopic();
    });

    this.root.querySelectorAll<HTMLButtonElement>("[data-lang-task-index]").forEach((button) => {
  ...
  private renderFeedback(task: LangTask, ui: LangCopy): string {
    if (this.status === "idle") {
      return "";
    }
