...
export function formatMathText(value: string): string {
  return value
    ...
    .replace(/\*/g, "·")
    .replace(/(?<=[\w)\]])\/(?=[\w([])/g, "⁄")
    .replace(/\^([+-]?\d+)/g, (_match, exponent: string) => exponentToSuperscript(exponent))
    .replace(/-/g, "−");
}

export function normalizeMathInput(value: string): string {
...
function exponentToSuperscript(exponent: string): string {
  return exponent
    .split("")
...
function replaceSuperscriptRuns(value: string): string {
